# Onboarding — SUPERSEDED

> **Use [`brand-onboarding.md`](brand-onboarding.md) instead.**
>
> This is upstream's single-skin flow. It ends by rewriting one global
> `style-guide.md`, and its closing note calls multi-brand support a future
> feature ("one skin per skill install"). This fork *is* that feature, so
> following the flow below would overwrite the registry with a single skin and
> reintroduce the exact failure the fork exists to prevent.
>
> The extraction technique here is still sound, and `brand-onboarding.md`
> inherits it — what changed is the destination (a per-client brand file), the
> verification (mechanical AA checks in both modes), and the refusal path (a
> named client with no brand file stops the run).

Retained below for reference only. Do not execute it as written.
