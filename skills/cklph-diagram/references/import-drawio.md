...
`<skill-dir>` is `skills/cklph-diagram/` in this repo, or the skill's own directory when it's installed standalone or as a plugin. If the path isn't obvious, glob for `**/cklph-diagram/scripts/drawio_extract.py`.
