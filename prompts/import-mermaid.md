...
Redraw Mermaid source at `$1`. Locate the installed `cklph-diagram` skill and read its `SKILL.md`. Then read `references/import-mermaid.md` and `references/output-spec.md` relative to its directory. Treat those references as the source of truth. Do not assume the package lives under the current working directory.
