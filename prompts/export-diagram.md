---
description: Export a cklph-diagram HTML file to SVG and PNG
argument-hint: "<html-file> [--svg-only|--png-only] [--scale=N] [--output=<path>]"
---

Export diagram HTML at `$1` to `.svg` or `.png`. Locate the installed `cklph-diagram` skill and read its `SKILL.md`. Then read `references/export.md` relative to its directory. Treat that reference as source of truth. Do not assume the package lives under the current working directory.
